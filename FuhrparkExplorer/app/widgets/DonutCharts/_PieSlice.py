...
from app.core.Utils import point_on_circle, font_metrics
...
class PieSlice(QObject, QPainterPath):
    ...
    def draw(self, painter: QPainter, start_angle: float, span_angle: float):
        ...
        painter.setFont(QFont("Roboto", int(size * 0.025)))
        if span_angle > 10:
    ...
    def __draw_text(self, painter: QPainter, text: str, text_width: int , text_height: int):
        if self.color.value() > 125:
            painter.setPen(QPen(QColor(0,0,0)))
        else:
            painter.setPen(QPen(QColor(255,255,255)))

        text_rect = QRect(0, int(-(text_height / 2)), text_width, text_height)
